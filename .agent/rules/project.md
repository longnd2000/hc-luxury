...
- **Nguyên tắc Chắp vá & Bảo tồn (Incremental Patching)**: Khi thực hiện các thay đổi (đặc biệt là trong `index.php` hoặc các module dùng chung), chỉ được phép thay đổi đúng các dòng cần thiết. Tuyệt đối không được xóa bỏ, sắp xếp lại hoặc thay đổi các đoạn code đang chạy đúng nếu không liên quan trực tiếp đến nhiệm vụ hiện tại.
