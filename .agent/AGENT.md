...
11. **Bảo tồn cấu trúc hiện có (CRITICAL)**: Khi chỉnh sửa hoặc bổ sung code, luôn ưu tiên thực hiện thay đổi cục bộ (chắp vá). Tuyệt đối không ghi đè (overwrite) toàn bộ file hoặc thay đổi cấu trúc đang hoạt động ổn định. Giữ nguyên quy chuẩn đặt tên, thứ tự đăng ký và logic của các thành phần cũ trừ khi có yêu cầu bắt buộc.
